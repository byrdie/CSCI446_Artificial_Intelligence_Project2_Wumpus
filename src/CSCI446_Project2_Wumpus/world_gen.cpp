...
World::World(int side_length, int n_wumpi, int n_pit, int n_barrier) {
    ...
    for (int i = 0; i < N + 2; i++) {
        ...
        for (int j = 0; j < N + 2; j++) {
            if (i == 0 or i == N + 1 or j == 0 or j == N + 1) {
                ...
                qt_world->set_tile(i, j, 1, WALL);
            } else {
                world_vec_row.push_back(EMPTY);
                qt_world->set_tile(i, j, 1, EMPTY);
            }
    ...
    for (int i = 0; i < num_wumpi; i++) {
        ...
        qt_world->set_tile(next_pt->x, next_pt->y, 5, WUMPUS);
    }
    ...
    for (int i = 0; i < num_pits; i++) {
        ...
        qt_world->set_tile(next_pt->x, next_pt->y, 2, PIT);
    }
    ...
    for (int i = 0; i < num_barriers; i++) {
        ...
        qt_world->set_tile(next_pt->x, next_pt->y, 2, WALL);
    }

    // Place the gold
    Point * next_pt = add_element(GOLD);
    gold_list.push_back(next_pt);
    qt_world->set_tile(next_pt->x, next_pt->y, 2, GOLD);
    ...
    for (int i = 0; i < num_wumpi; i++) {
        ...
        for(uint j = 0; j < affected_tiles.size(); j++){
            ...
            qt_world->set_tile(tile->x, tile->y, 4, STENCH);
        }
    }
    ...
    for(int i = 0; i < num_pits; i++){
        ...
        for(uint j = 0; j < affected_tiles.size(); j++){
            Point * tile = affected_tiles[j];
            qt_world->set_tile(tile->x, tile->y, 3, BREEZE);
        }
    }
...
vector<Point *> World::add_effect(Point * center, int effect_bits){
    ...
    for(int i = 0; i < 4; i++){
        ...
        if(tile != WALL){
            ...
            qt_world->set_tile(pt->x, pt->y, 3, effect_bits);
        } else {
